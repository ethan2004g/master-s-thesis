...
Future work should sweep $\lambda_\mathrm{bel}$ and try belief fusion into the
policy input (\Cref{ch:model}) rather than parallel heads only.
...
conditioned transformers infer unrevealed moves, not that the full hidden-state
posterior is solved.

\section{Decision Tree and Attention Studies}

Shallow decision trees on hand-crafted turn features and attention visualizations
were planned to compare interpretable clues against neural errors.
\todo{Train decision trees on damage hints, hazard state, and revealed moves;
  report feature importances versus transformer move-belief errors.}

\todo{Qualitative case studies on turns where move belief becomes correct before
  official reveal; inspect slot attention weights.}

Expected error modes remain relevant for future case studies
\begin{itemize}
...
parallel heads alone (19.6\% versus 20.2\% Split~A). Flat-sequence ablations
remain open.
