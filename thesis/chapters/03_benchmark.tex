...
battle start in Gen~9 OU, team preview reveals all opponent species.
Unrevealed moves, items, abilities, and terastallization type remain hidden and
...
The implementation roadmap for this draft is complete for Stage~A offline
experiments.
\begin{enumerate}
  ...
  \item Metamon bridge and belief-enriched export \textbf{(complete)}.
  \item Multi-task belief training and matchup ablations \textbf{(complete)}.
  \item Split~A / Split~B combinatorial evaluation harness \textbf{(complete)}.
  \item Live poke-env win-rate appendix and Split~C \textbf{(deferred)}.
\end{enumerate}
